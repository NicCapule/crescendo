...
const ChatButton = ({ formValues }) => {
  ...
  const [messages, setMessages] = useState([
    {
      sender: "ai",
      text: "Hi! Need help with your enrollment or scheduling? I can assist with finding available time slots based on your preferences.",
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const studentId =
        formValues?.student_id || (formValues?.isNewStudent ? "new" : null);

      const response = await axios.post("http://127.0.0.1:5173/chat", {
        prompt: input,
  ...
  const getSuggestions = () => {
    ...
    if (formValues?.instrument_name) {
      suggestions.push(
        `What's the best schedule for ${formValues.instrument_name} lessons?`
      );
    }
    ...
    if (formValues?.availability && formValues?.availability.length > 0) {
      suggestions.push(
        "Can you suggest some good time slots based on my availability?"
      );
    }
