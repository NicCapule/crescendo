"use strict";
const bcrypt = require("bcryptjs");
/** @type {import('sequelize-cli').Migration} */
module.exports = {
  async up(queryInterface, Sequelize) {
    const salt = await bcrypt.genSalt(10);
    await queryInterface.bulkInsert("User", [
      {
        email: "admin@example.com",
        user_first_name: "Michael",
        user_last_name: "Scott",
        password: await bcrypt.hash("password", salt),
        role: "Admin",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        email: "jake@gmail.com",
        user_first_name: "Jake",
        user_last_name: "Peralta",
        password: await bcrypt.hash("password", salt),
        role: "Teacher",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        email: "amy@gmail.com",
        user_first_name: "Amy",
        user_last_name: "Santiago",
        password: await bcrypt.hash("password", salt),
        role: "Teacher",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        email: "jim@gmail.com",
        user_first_name: "Jim",
        user_last_name: "Halpert",
        password: await bcrypt.hash("password", salt),
        role: "Teacher",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        email: "pam@gmail.com",
        user_first_name: "Pam",
        user_last_name: "Beesly",
        password: await bcrypt.hash("password", salt),
        role: "Teacher",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        email: "doug@gmail.com",
        user_first_name: "Doug",
        user_last_name: "Judy",
        password: await bcrypt.hash("password", salt),
        role: "Teacher",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        email: "rosa@gmail.com",
        user_first_name: "Rosa",
        user_last_name: "Diaz",
        password: await bcrypt.hash("password", salt),
        role: "Teacher",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
    ]);

    await queryInterface.bulkInsert("Instrument", [
      {
        instrument_name: "Piano",
      },
      {
        instrument_name: "Guitar",
      },
      {
        instrument_name: "Violin",
      },
      {
        instrument_name: "Drums",
      },
      {
        instrument_name: "Ukulele",
      },
      {
        instrument_name: "Voice",
      },
    ]);

    await queryInterface.bulkInsert("Teacher", [
      {
        user_id: 2,
        teacher_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        user_id: 3,
        teacher_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        user_id: 4,
        teacher_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        user_id: 5,
        teacher_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        user_id: 6,
        teacher_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        user_id: 7,
        teacher_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
    ]);

    await queryInterface.bulkInsert("TeacherInstrument", [
      {
        teacher_id: 1,
        instrument_id: 1,
      },
      {
        teacher_id: 1,
        instrument_id: 2,
      },
      {
        teacher_id: 1,
        instrument_id: 3,
      },
      {
        teacher_id: 2,
        instrument_id: 4,
      },
      {
        teacher_id: 3,
        instrument_id: 1,
      },
      {
        teacher_id: 3,
        instrument_id: 5,
      },
      {
        teacher_id: 4,
        instrument_id: 6,
      },
      {
        teacher_id: 5,
        instrument_id: 2,
      },
      {
        teacher_id: 5,
        instrument_id: 3,
      },
      {
        teacher_id: 6,
        instrument_id: 4,
      },
      {
        teacher_id: 6,
        instrument_id: 5,
      },
    ]);

    await queryInterface.bulkInsert("Student", [
      {
        student_first_name: "Russel",
        student_last_name: "Segador",
        student_address: "Marikina",
        student_age: 22,
        student_email: "teytesting@gmail.com",
        student_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        student_first_name: "Angelica",
        student_last_name: "Babon",
        student_address: "Marikina",
        student_age: 22,
        student_email: "teytesting@gmail.com",
        student_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        student_first_name: "Mikee",
        student_last_name: "Reyes",
        student_address: "Marikina",
        student_age: 30,
        student_email: "teytesting@gmail.com",
        student_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        student_first_name: "CK",
        student_last_name: "De Leon",
        student_address: "Marikina",
        student_age: 23,
        student_email: "teytesting@gmail.com",
        student_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        student_first_name: "Exekiel",
        student_last_name: "Gaspar",
        student_address: "Marikina",
        student_age: 30,
        student_email: "teytesting@gmail.com",
        student_phone: "09123456789",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
    ]);

    // await queryInterface.bulkInsert("Room", [
    //   {
    //     room_name: "Piano Room 1",
    //     has_piano: true,
    //     has_drums: false,
    //   },
    //   {
    //     room_name: "Piano Room 2",
    //     has_piano: true,
    //     has_drums: false,
    //   },
    //   {
    //     room_name: "Piano Room 3",
    //     has_piano: true,
    //     has_drums: false,
    //   },
    //   {
    //     room_name: "Drum Room",
    //     has_piano: true,
    //     has_drums: true,
    //   },
    // ]);

    await queryInterface.bulkInsert("Program", [
      {
        teacher_id: 1,
        instrument_id: 1,
        no_of_sessions: 8,
        program_status: "Active",
      },
      {
        teacher_id: 2,
        instrument_id: 4,
        no_of_sessions: 16,
        program_status: "Active",
      },
      {
        teacher_id: 3,
        instrument_id: 5,
        no_of_sessions: 8,
        program_status: "Active",
      },
      {
        teacher_id: 4,
        instrument_id: 6,
        no_of_sessions: 8,
        program_status: "Active",
      },
      {
        teacher_id: 5,
        instrument_id: 3,
        no_of_sessions: 8,
        program_status: "Active",
      },
    ]);

    await queryInterface.bulkInsert("Enrollment", [
      {
        student_id: 1,
        program_id: 1,
        student_name: "Russel Segador",
        student_email: "teytesting@gmail.com",
        teacher_name: "Jake Peralta",
        instrument: "Piano",
        enroll_date: new Date(),
        total_fee: 7000.0,
        payment_status: "Unsettled",
      },
      {
        student_id: 2,
        program_id: 2,
        student_name: "Angelica Babon",
        student_email: "teytesting@gmail.com",
        teacher_name: "Amy Santiago",
        instrument: "Drums",
        enroll_date: new Date(),
        total_fee: 12000.0,
        payment_status: "Unsettled",
      },
      {
        student_id: 3,
        program_id: 3,
        student_name: "Mikee Reyes",
        student_email: "teytesting@gmail.com",
        teacher_name: "Jim Halpert",
        instrument: "Ukulele",
        enroll_date: new Date(),
        total_fee: 7000.0,
        payment_status: "Unsettled",
      },
      {
        student_id: 4,
        program_id: 4,
        student_name: "CK De Leon",
        student_email: "teytesting@gmail.com",
        teacher_name: "Pam Beesly",
        instrument: "Voice",
        enroll_date: new Date(),
        total_fee: 7000.0,
        payment_status: "Unsettled",
      },
      {
        student_id: 5,
        program_id: 5,
        student_name: "Exekiel Gaspar",
        student_email: "teytesting@gmail.com",
        teacher_name: "Doug Judy",
        instrument: "Violin",
        enroll_date: new Date(),
        total_fee: 7000.0,
        payment_status: "Unsettled",
      },
    ]);

    await queryInterface.bulkInsert("Session", [
      {
        student_id: 1,
        program_id: 1,
        session_number: 1,
        session_date: "2025-03-15",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 2,
        session_date: "2025-03-22",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 3,
        session_date: "2025-03-29",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 4,
        session_date: "2025-04-5",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 5,
        session_date: "2025-04-12",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 6,
        session_date: "2025-04-19",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 7,
        session_date: "2025-04-26",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 1,
        program_id: 1,
        session_number: 8,
        session_date: "2025-05-03",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      //===============================================//
      {
        student_id: 2,
        program_id: 2,
        session_number: 1,
        session_date: "2025-03-16",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },

      {
        student_id: 2,
        program_id: 2,
        session_number: 2,
        session_date: "2025-03-16",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 3,
        session_date: "2025-03-23",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 4,
        session_date: "2025-03-23",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 5,
        session_date: "2025-03-30",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 6,
        session_date: "2025-03-30",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 7,
        session_date: "2025-04-06",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 8,
        session_date: "2025-04-06",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 9,
        session_date: "2025-04-13",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 10,
        session_date: "2025-04-13",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 11,
        session_date: "2025-04-20",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 12,
        session_date: "2025-04-20",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 13,
        session_date: "2025-04-27",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 14,
        session_date: "2025-04-27",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 15,
        session_date: "2025-05-04",
        session_start: "11:00:00",
        session_end: "12:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 2,
        program_id: 2,
        session_number: 16,
        session_date: "2025-05-04",
        session_start: "12:00:00",
        session_end: "13:00:00",
        session_status: "Scheduled",
      },
      //===============================================//
      {
        student_id: 3,
        program_id: 3,
        session_number: 1,
        session_date: "2025-03-15",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 2,
        session_date: "2025-03-22",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 3,
        session_date: "2025-03-29",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 4,
        session_date: "2025-04-5",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 5,
        session_date: "2025-04-12",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 6,
        session_date: "2025-04-19",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 7,
        session_date: "2025-04-26",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 3,
        program_id: 3,
        session_number: 8,
        session_date: "2025-05-03",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      //===============================================//
      {
        student_id: 4,
        program_id: 4,
        session_number: 1,
        session_date: "2025-03-15",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 2,
        session_date: "2025-03-22",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 3,
        session_date: "2025-03-29",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 4,
        session_date: "2025-04-5",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 5,
        session_date: "2025-04-12",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 6,
        session_date: "2025-04-19",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 7,
        session_date: "2025-04-26",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 4,
        program_id: 4,
        session_number: 8,
        session_date: "2025-05-03",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      //===============================================//
      {
        student_id: 5,
        program_id: 5,
        session_number: 1,
        session_date: "2025-03-15",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 2,
        session_date: "2025-03-22",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 3,
        session_date: "2025-03-29",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 4,
        session_date: "2025-04-5",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 5,
        session_date: "2025-04-12",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 6,
        session_date: "2025-04-19",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 7,
        session_date: "2025-04-26",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
      {
        student_id: 5,
        program_id: 5,
        session_number: 8,
        session_date: "2025-05-03",
        session_start: "08:00:00",
        session_end: "09:00:00",
        session_status: "Scheduled",
      },
    ]);

    await queryInterface.bulkInsert("StudentAvailability", [
      {
        student_id: 1,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "20:00:00",
      },
      {
        student_id: 2,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "20:00:00",
      },
    ]);

    await queryInterface.bulkInsert("TeacherAvailability", [
      {
        teacher_id: 1,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 1,
        day_of_week: "Wednesday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 1,
        day_of_week: "Saturday",
        start_time: "08:00:00",
        end_time: "12:00:00",
      },
      {
        teacher_id: 2,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 2,
        day_of_week: "Wednesday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 2,
        day_of_week: "Sunday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 3,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 3,
        day_of_week: "Wednesday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 3,
        day_of_week: "Saturday",
        start_time: "08:00:00",
        end_time: "12:00:00",
      },
      {
        teacher_id: 4,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 4,
        day_of_week: "Wednesday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 4,
        day_of_week: "Saturday",
        start_time: "08:00:00",
        end_time: "12:00:00",
      },
      {
        teacher_id: 5,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 5,
        day_of_week: "Wednesday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 5,
        day_of_week: "Saturday",
        start_time: "08:00:00",
        end_time: "12:00:00",
      },
      {
        teacher_id: 6,
        day_of_week: "Monday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 6,
        day_of_week: "Wednesday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
      {
        teacher_id: 6,
        day_of_week: "Sunday",
        start_time: "08:00:00",
        end_time: "15:00:00",
      },
    ]);

    await queryInterface.bulkInsert("StudentPayment", [
      {
        enrollment_id: 1,
        student_name: `Russel Segador`,
        amount_paid: 4000.0,
        payment_method: "Cash",
        student_payment_date: new Date(),
      },
      {
        enrollment_id: 2,
        student_name: `Angelica Babon`,
        amount_paid: 8000.0,
        payment_method: "GCash",
        student_payment_date: new Date(),
      },
    ]);

    // await queryInterface.bulkInsert("TeacherSalary", [
    //   {
    //     teacher_id: 1,
    //     teacher_name: "Jake Peralta",
    //     total_sessions: 2,
    //     salary_date: new Date(),
    //     amount: 1000.0,
    //   },
    //   {
    //     teacher_id: 2,
    //     teacher_name: "Amy Santiago",
    //     total_sessions: 2,
    //     salary_date: new Date(),
    //     amount: 1000.0,
    //   },
    // ]);
  },

  async down(queryInterface, Sequelize) {
    await queryInterface.bulkDelete("TeacherSalary", null, {});
    await queryInterface.bulkDelete("StudentPayment", null, {});
    await queryInterface.bulkDelete("TeacherAvailability", null, {});
    await queryInterface.bulkDelete("StudentAvailability", null, {});
    await queryInterface.bulkDelete("Session", null, {});
    await queryInterface.bulkDelete("Enrollment", null, {});
    await queryInterface.bulkDelete("Program", null, {});
    await queryInterface.bulkDelete("Room", null, {});
    await queryInterface.bulkDelete("Student", null, {});
    await queryInterface.bulkDelete("TeacherInstrument", null, {});
    await queryInterface.bulkDelete("Teacher", null, {});
    await queryInterface.bulkDelete("Instrument", null, {});
    await queryInterface.bulkDelete("User", null, {});
  },
};
